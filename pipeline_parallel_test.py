...
from faket_polnet.utils.utils import collect_results_to_train_dir
...
def parse_args():
    ...
    # Other parameters
    parser.add_argument('--detector_snr', type=float, nargs=2, default=[0.15, 0.20], 
                        help='Detector SNR range')
    parser.add_argument('--lowpass', type=float, default=0.0,
                        help='Lowpass filter frequency (A) applied to synhetic density.')
    parser.add_argument('--denoised', action='store_true',
                        help='Use denoised style micrographs')
    parser.add_argument('--style_seed', type=int, default=None,
                        help='Seed for reproducible random style selection order.')
...
def run_setup(args, base_dir, style_dir, style_tomo_dir, style_tomo_exists, style_dir_exists, faket_end_scale=None):
    ...
    if style_dir_exists:
        ...
        project_style_micrographs(style_tomo_dir, style_dir, tilt_range=tilt_range,
                                   ax="Y", target_shape=target_shape)
        print(f"Style projection completed to: {style_dir}")
    else:
...
def run_style_transfer(tomo_index, args, base_dir, style_dir, faket_end_scale):
    ...
    if args.style_seed is not None:
        style_paths = sorted(str(p) for p in Path(style_dir).glob("*_style_mics.mrc"))
        if not style_paths:
        ...
        STYLE_TOMOGRAM_CMD = f"find {style_dir} -name '*_style_mics.mrc' | shuf -n 1"
        STYLE_TOMOGRAM = subprocess.run(
            STYLE_TOMOGRAM_CMD, shell=True, capture_output=True, text=True
        ).stdout.strip()

    style_name = Path(STYLE_TOMOGRAM).stem.replace('_style_mics', '')
