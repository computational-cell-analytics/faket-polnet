...
def project_content_micrographs(out_base_dir, simulation_dir, tilt_range=(-60, 60, 3), detector_snr=None, simulation_index=None,
    ...
    Returns:
        snr (float or None): SNR value used for the projected tomogram.
    """
    MALIGN_MN = 1
    MALIGN_MX = 1.5
    MALIGN_SG = 0.2
    ...
    if not os.path.exists(simulation_dir):
        print(f"Simulation directory {simulation_dir} does not exist. Skipping.")
        return snr

    if not os.listdir(simulation_dir):
        print(f"Simulation directory {simulation_dir} is empty. Skipping.")
        return snr
    ...
    for i in range(n_tomos):
        ...
        tem_output_dir = os.path.join(out_base_dir, f"TEM/tomogram_{simulation_index}_{i}")
        os.makedirs(tem_output_dir, exist_ok=True)

        # Create output directory for micrographs
        tomo_output_dir = os.path.join(out_base_dir, f"Micrographs/tomogram_{simulation_index}_{i}")
        ...
        out_mics = os.path.join(tomo_output_dir, f"tomo_mics_{i}_{snr}.mrc")
...
def reconstruct_micrographs_only_recon3D(TEM_path, faket_path, out_base_dir, snr, custom_mic=False,
                                         cluster_run=False):
    os.makedirs(out_base_dir, exist_ok=True)

    if not os.path.exists(TEM_path):
        print(f"TEM directory {TEM_path} does not exist. Skipping.")
        return
    ...
    if custom_mic:
        if not os.path.exists(faket_path):
            raise FileNotFoundError(f"Micrographs file {faket_path} does not exist.")
        temic._TEM__micgraphs_file = faket_path
